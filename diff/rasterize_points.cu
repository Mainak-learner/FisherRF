	...
	const bool debug)
{
  ...
  torch::Tensor out_entropy = torch::full({H, W}, 0.0, float_opts);
...
//   return std::make_tuple(rendered, out_color, out_depth, radii, geomBuffer, binningBuffer, imgBuffer);
  return std::make_tuple(rendered, out_color, out_depth, radii, geomBuffer, binningBuffer, imgBuffer, pixel_gaussian_counter, out_entropy);
}
